...
public class PlayerVSAIGameScript : MonoBehaviour
{
        ...
        if (timePassedSinceFirstUpdate > 0.125f) {
            ...
            if (!tieChecker) {
                if (!winChecker) {
                    ...
                    GameObject newBoard = new GameObject("Player VS AI Game script " + (boardNum + 1), typeof(PlayerVSAIGameScript));
                    PlayerVSAIGameScript newBoardScript = newBoard.GetComponent<PlayerVSAIGameScript>();
                    newBoardScript.setBoardNum(boardNum + 1);
                    newBoardScript.setCameras(mainCamera, otherCamera);
                    Destroy(GameObject.Find("Player VS AI Game script " + boardNum));
                    win = true;
                }
            }
            else {
    ...
    //                     Destroy(boardComponents[boardItem]);
    //                 }
    //                 GameObject newBoard = new GameObject("Player VS AI Game script " + (boardNum + 1), typeof(PlayerVSPlayerGameScript));
    ...
    private void createTicTacToeBoard() {
        ...
        for (int rows = 0; rows < boxHeight; rows++) {
            for (int columns = 0; columns < boxWidth; columns++) {
                GameObject block = new GameObject("Block (" + rows + ", " + columns + ") " + boardNum, typeof(SpriteRenderer));
    ...
    public void placingMarksAndUpdatingBoard(float xPos, float yPos) {
        ...
        if ((clickLocater.transform.position.x >= -1*(boxWidth*2.5)/2 && clickLocater.transform.position.x <= (boxWidth*2.5)/2) && (clickLocater.transform.position.y >= -1*(boxHeight*2.5)/2 && clickLocater.transform.position.y <= (boxHeight*2.5)/2)) {
            ...
            GameObject selectedBlock = GameObject.Find("Block (" + squaresDown + ", " + squaresRight + ") " + (boardNum));
    ...
    private bool checkForWin(GameObject coords, int heightDiff, int widthDiff) {
        ...
        if (turnDecider % 2 == 0) {
            while (keepGoing1) {
                if (xPlacements.Contains(GameObject.Find("Block (" + (squaresDown + (heightDiff * multiplier)) + ", " + (squaresRight + (widthDiff * multiplier)) + ") " + boardNum))) {
                    winningConnectors.Add(GameObject.Find("Block (" + (squaresDown + (heightDiff * multiplier)) + ", " + (squaresRight + (widthDiff * multiplier)) + ") " + boardNum));
                    connectors++;
                    multiplier++;
                }
                else {
            ...
            while (keepGoing1) {
                if (oPlacements.Contains(GameObject.Find("Block (" + (squaresDown + (heightDiff * multiplier)) + ", " + (squaresRight + (widthDiff * multiplier)) + ") " + boardNum))) {
                    winningConnectors.Add(GameObject.Find("Block (" + (squaresDown + (heightDiff * multiplier)) + ", " + (squaresRight + (widthDiff * multiplier)) + ") " + boardNum));
                    connectors++;
                    multiplier++;
                }
    ...
    private void setNumOfDeadSpaces() {
        if (boardNum == 0) {
            ...
            for (int i = 0; i < numOfDeadSpaces; i++) {
                ...
                deadSpacesCoords.Add("Block (" + yIndex + ", " + xIndex + ") " + boardNum);
            ...
            while (deadCounter < numOfDeadSpaces) {
                ...
                string sampleText = "Block (" + randomYCoord + ", " + randomXCoord + ") " + boardNum;
    ...
    public Sprite getMarkOne() {
        return mark1;
    }

    public Sprite getMarkTwo() {
        return mark2;
    }
    ...
    public float getTimePassed() {
        return timePassedSinceFirstUpdate;
    }
